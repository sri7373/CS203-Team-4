        ...
        <div className="right">
          ...
          {loggedIn && (
            <div className="user-meta" aria-live="polite">
              <span className="badge" style={{background: 'var(--color-primary)'}}>{user?.role}</span> {user?.username}
            </div>
          )}
          {loggedIn && <button type="button" onClick={doLogout} className="danger" aria-label="Logout">Logout</button>}
        </div>
      </div>
    </nav>
  )
